...
import com.zxd.mq.conf.OrderPoolConfig;
...
public class OrderThreadPool {
    ...
    private final static Logger logger = LoggerFactory.getLogger(OrderThreadPool.class);

    @Autowired
    private OrderPoolConfig orderPoolConfig;
    ...
    public void test() throws Exception {
        ExecutorService pool = orderPoolConfig.buildOrderQueueThreadPool();
        long start = System.currentTimeMillis();
        for (int i = 1; i <= 15; i++) {
            pool.execute(new OrderThreadPool.MyRunable(i));
        }
